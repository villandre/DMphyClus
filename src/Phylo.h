...
typedef adjacency_list<vecS, vecS, bidirectionalS, VertexProperties, no_property, no_property, listS> DirectedGraph;
...
protected:
  ...
  std::vector<std::vector<Mat<long double>>> alignmentBin ;
  uint numStates;
  uint numRateCats;
...
public:
  phylo(const NumericMatrix &, const NumericVector &, const List &, const int, const bool, const uvec &) ;
  phylo() ;
