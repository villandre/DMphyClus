...
logLikFromClusInd <- function(phylogeny, betweenTransMatList, withinTransMatList, clusInd, limProbs, numLikThreads, alignment, basicChecks = TRUE) {
    ...
    logLikAndPointer <- logLikCpp(edgeMat = phylogeny$edge, clusterMRCAs = clusMRCAs, limProbsVec = limProbs, withinTransMatList = withinTransMatList, betweenTransMatList = betweenTransMatList, numThreads = numLikThreads, alignmentBin = alignmentBin, numTips = ape::Ntip(phylogeny), numLoci = ncol(alignment))
